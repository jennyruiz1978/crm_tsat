...
class ModeloFacturasCliente{
    ...
    public function obtenerDatosFactura($idFactura){
        $this->db->query("SELECT fac.*, (SELECT cli.cif FROM clientes cli WHERE fac.idcliente = cli.id ) as cif FROM facturasclientes fac WHERE fac.id = '$idFactura' ");                
    ...
    public function obtenerDatosFacturaYCliente($idFactura){
        $this->db->query("SELECT fac.*, cli.direccion, cli.codigopostal, cli.poblacion, 
                        cli.provincia, cli.cif, cli.cuentas
                        FROM facturasclientes fac 
    ...
    public function registrosAdminTablaClassBuscar($filas,$orden,$filaspagina,$tipoOrden,$cond){

        $this->db->query("SELECT fac.id as 'Nº', 
                        fac.numero AS 'Numero factura', 
                        fac.cliente,  
                        IF(cli.cif is not null, cli.cif,'' ) as cif,
                        DATE_FORMAT(fac.fecha, '%d/%m/%Y', 'es_ES') AS fecha,  
                        IF(fac.vencimiento > 0,DATE_FORMAT(fac.vencimiento, '%d/%m/%Y', 'es_ES'),'') AS vencimiento, fac.estado
                        FROM facturasclientes fac  
                        LEFT JOIN clientes cli ON fac.idcliente=cli.id                       
                        WHERE $cond
    ...
    public function totalRegistrosBuscar($cond)
    {
    ...
    public function estadoPagoFactura($idFactura)
    {
        $this->db->query("SELECT estado
                        FROM facturasclientes fac                        
                        WHERE id='$idFactura' ");
                      
        $fila = $this->db->registro();
        $estado = ($fila->estado != null && $fila->estado != '')? $fila->estado:false;
        return $fila->estado;
    }

 

    public function actualizarEstadoPagoFactura($idFactura,$estado)
    {               

        $this->db->query("UPDATE facturasclientes 
                        SET estado = '$estado'                        
                        WHERE id = '$idFactura' ");
    ...
    public function insertHeaderInvoice($datos)
    {     
        $this->db->query("INSERT INTO facturasclientes (numero,numerointerno,idcliente,cliente,fecha,vencimiento,observaciones,estado,idcuentabancaria,idformacobro,diascobro,rectificativa) VALUES (:numero,:numerointerno,:idcliente,:cliente,:fecha,:vencimiento,:observaciones,:estado,:idcuentabancaria,:idformacobro,:diascobro,:rectificativa)");      
        
        $this->db->bind(':numero', $datos['numero']);
        $this->db->bind(':numerointerno', $datos['numerointerno']);
        ...
        $this->db->bind(':idcuentabancaria', $datos['idcuentabancaria']);
        $this->db->bind(':idformacobro', $datos['idformacobro']);
        $this->db->bind(':diascobro', $datos['diascobro']);
        $this->db->bind(':rectificativa', $datos['rectificativa']);
    ...
    public function updateHeaderInvoice($datos)
    {
        $this->db->query("UPDATE facturasclientes 
                        SET idcliente= :idcliente,cliente= :cliente,fecha= :fecha,vencimiento= :vencimiento,observaciones= :observaciones,estado= :estado,idcuentabancaria= :idcuentabancaria,idformacobro= :idformacobro,diascobro= :diascobro
                        WHERE id = :id ");
    ...
    {        
        ...
        $estado = $datos['estado'];
        $activo = $datos['activo'];
        $tecnicosAsig = $datos['tecnicos'];
        $nombresTecnicos = $datos['nombresTecnicos'];  
        $ip = $datos['remoteADDR'];
        $estadofactppto = $datos['estadofactppto'];
        $nomestadofactppto = $datos['nomestadofactppto'];
        
        $this->db->query("INSERT INTO incidencias (descripcion, idusuario, idcliente, sucursal, creacion, idequipo, estado, activo, tecnicos, nombrestecnicos,ipincidencia,estadofactppto,nomestadofactppto) 
                        VALUES ('$descripcion', '$idusuario', '$idcliente', '$idsucursal', '$creacion', '$idequipo', '$estado', '$activo', '$tecnicosAsig', '$nombresTecnicos', '$ip', '$estadofactppto', '$nomestadofactppto' )"); 
    ...
    {
        $this->db->query("SELECT tmp.*, moda.modalidad FROM incidenciastiempos tmp 
    ...
    public function nuFacturaPorIdFactura($idFactura)
    {
        $this->db->query("SELECT numero FROM facturasclientes WHERE id = '$idFactura' ");
        $fila = $this->db->registro();
        return (isset($fila->numero) && $fila->numero != '')?$fila->numero:false;
    }

    public function obtenerFacturasPorIdCliente($idCliente)
    {
        $this->db->query("SELECT id, numero, fecha, total FROM facturasclientes WHERE idcliente = '$idCliente' ");
        $filas = $this->db->registros();
        return $filas;
    }

    public function idClientePorIdFactura($idFactura)
    {
        $this->db->query("SELECT idcliente FROM facturasclientes WHERE id = '$idFactura' ");
        $fila = $this->db->registro();
        return (isset($fila->idcliente) && $fila->idcliente > 0)? $fila->idcliente:false;
    }

    public function eliminarCabeceraFactura($idFactura)
    {
        $this->db->query("DELETE FROM facturasclientes WHERE id = '$idFactura' ");
    ...
    public function verificarSiExistenFacturasPosteriores($idFactura, $anioFactura, $numerointerno, $rectificativa)
    {
        $rectificativaCond = ($rectificativa == 1) ? "rectificativa = 1" : "(rectificativa = 0 OR rectificativa IS NULL)";

        $this->db->query("SELECT COUNT(*) AS total 
                        FROM facturasclientes 
                        WHERE YEAR(fecha) = '$anioFactura'
                        AND $rectificativaCond
    ...
    public function actualizarClienteFactura($idFactura, $idClienteNuevo, $clienteNuevo)
    {
        $this->db->query("UPDATE facturasclientes
                        SET idcliente = :idcliente, cliente = :cliente
                        WHERE id = :idFactura");

        $this->db->bind(':idcliente', $idClienteNuevo);
        $this->db->bind(':cliente', $clienteNuevo);
