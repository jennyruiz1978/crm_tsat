    ...
    if(window.location.pathname.includes('/Clientes')){
        ...
        $(document).on('click', '#crearClienteNuevo', function (e) {
            ...
            if (document.getElementById("formAltaClientes").checkValidity()) {
                ...
                var cuentas = $("#cuentas-bancarias-list input[name='selectCuenta[]']");

                // 🔥 Solo agregar al formulario si existen cuentas bancarias
                if (cuentas.length > 0) {
                    cuentas.each(function () {
                        $("#formAltaClientes").append($(this).clone());
                    });
                }
            ...
            if (document.getElementById("formAltaClientes").checkValidity()) {               
                ...
                $.ajax({
                    ...
                    if (data['error'] == false) {                        
                        ...
                        $('#tabla1tbody').find(clienteEdicion).find('td:eq(2)').html(nombrecomercial);
                        $('#tabla1tbody').find(clienteEdicion).find('td:eq(3)').html(cif);
                        $('#tabla1tbody').find(clienteEdicion).find('td:eq(4)').html(poblacion);
                        $('#tabla1tbody').find(clienteEdicion).find('td:eq(5)').html(provincia);
                        Swal.fire({
        ...
        //nuevo cuentas bancarias clientes
        var cuentaRegex = /^[A-Z]{2}\d{2}[A-Z0-9\s]{4,30}$/; // Expresión regular corregida para IBAN

        $(document).on("click", "#agregarCuenta", function () {
            var cuenta = $("#nuevaCuenta").val().trim();
            ...
            if (!cuentaRegex.test(cuenta)) {
                alert("Formato de número de cuenta inválido. Asegúrese de ingresar un IBAN válido, solo números y letras.");
                return;
            }
    
            var cuentaId = Date.now(); // Genera un ID único
            var cuentaHTML = `
                <div class="flex justify-between items-center mb-2" id="cuenta-${cuentaId}">
                    <span>${cuenta}</span>
                    <input type="hidden" name="selectCuenta[]" value="${cuenta}">
                    <button type="button" class="text-red-500 text-xs eliminarCuenta" data-id="${cuentaId}">Eliminar</button>
                </div>
            `;
            $("#cuentas-bancarias-list").append(cuentaHTML);
            $("#nuevaCuenta").val("").removeClass("border-green-500 border-red-500"); // Limpiar input y clases de validación
        });
        
        // Delegación del evento para eliminar cuenta
