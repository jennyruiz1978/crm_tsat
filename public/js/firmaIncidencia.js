...
if(window.location.pathname.includes('/Incidencias')){
    ...
    if(document.querySelector("#canvas")){
        ...
        // Capturar imagen y enviarla a PHP como base64
        const $botonCapturarImagen = document.querySelector("#btnCapturarImagen");

        $botonCapturarImagen.addEventListener("click", () => {
            const imagenBase64 = $canvas.toDataURL("image/png");

            $.ajax({
                type: 'POST',
                url: urlCompleta + '/Incidencias/guardarFirma',
                dataType: "json",
                data: {
                    imagen: imagenBase64,
                    idIncidencia: document.getElementById('idIncidenciaVer').value
                }
            }).done(function(data) {
                if (data.respuesta == 1) {               
                    document.getElementById('firma_container').innerHTML = data.html;
                    
                    Swal.fire({
                        title: 'Firma guardada',
                        text: 'Se ha guardado la firma corréctamente',
                        icon: 'success',
                ...
                console.error("Error al enviar la firma: ", textStatus, errorThrown);
            });
        });
